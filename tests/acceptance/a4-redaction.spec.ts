...
describe('A4: secrets redacted at capture, plaintext never stored, deterministic within session', () => {
  ...
  test('redaction catches known secret patterns and produces identical hashes for same secret within session', async () => {
    ...
    // No plaintext
    expect(dbText, 'Anthropic key must not appear plaintext in DB').not.toContain('sk-ant-api03-TEST-SECRET-ABCDEFGHIJ1234567890abcdefghij012345');
    expect(dbText, 'AWS key must not appear plaintext in DB').not.toContain('AKIAIOSFODNN7EXAMPLE');
    expect(dbText, 'env-var heuristic value must not appear plaintext').not.toContain('sk-foo-bar-baz-not-matching-standard-pattern-1234567890');
    ...
    expect(match1![1], 'same secret → same hash within session (deterministic)').toEqual(match2![1]);
