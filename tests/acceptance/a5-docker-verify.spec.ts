...
const SKIP =
  ...
  !existsSync('docker/Dockerfile.verify');

describe.skipIf(SKIP)('A5: fresh Docker container imports fixture and verifies token counts', () => {
  test('peek CLI inside Docker imports fixtures and passes verify', async () => {
    await dockerBuild('docker/Dockerfile.verify');
    const result = await dockerRun({
      cmd: ['peek', 'import', '/fixtures/isolated-claude-projects/', '&&', 'peek', 'verify', '/fixtures/expected-counts.json'],
      volumes: {
    ...
    expect(result.exitCode, `Docker verify must exit 0. stdout: ${result.stdout}\nstderr: ${result.stderr}`).toBe(0);
